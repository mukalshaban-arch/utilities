...
def test_pg_dump_path_raises_when_not_found(app):
    app.config["PG_DUMP"] = None
    with (
        patch("app.backup.shutil.which", return_value=None),
        patch("app.backup.Path.glob", return_value=iter([])),
        pytest.raises(BackupError, match="pg_dump was not found"),
    ):
        pg_dump_path()
...
def test_create_backup_invokes_pg_dump_safely(pg_app):
    ...
    with (
        patch("app.backup.pg_dump_path", return_value="pg_dump"),
...
def test_create_backup_reports_a_failed_dump(pg_app):
    ...
    with (
        ...
        pytest.raises(BackupError, match="role does not exist"),
    ):
        create_backup()
...
def test_create_backup_reports_a_missing_executable(pg_app):
    with (
        ...
        pytest.raises(BackupError, match="Could not run pg_dump"),
    ):
        create_backup()
